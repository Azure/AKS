...
* Fixed [issue](https://github.com/cilium/cilium/issues/XXXX) where [Azure CNI Powered by Cilium](https://learn.microsoft.com/azure/aks/azure-cni-powered-by-cilium) DNS request/response performance degraded in large scale clusters using FQDN based policies. Without this fix, DNS request timeouts below 2 seconds may experience request drops due to duplicate request IDs.
