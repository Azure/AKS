...
![Cartoon with Key Vault, Istio, and DNS](./Designer.jpg)
...
Because the logs land on stdout, they flow through the standard container log pipeline, so you can collect them centrally instead of tailing each gateway by hand. Enable the **Azure Monitor (Container Insights) add-on**, and each gateway proxy's access logs are collected into a [Log Analytics workspace](https://learn.microsoft.com/azure/azure-monitor/logs/log-analytics-workspace-overview), where you can query, retain, and alert on them with KQL. You can enable it when you create or update the cluster (`az aks create/update --enable-addons monitoring --workspace-resource-id <workspace-id>`), or onboard an existing cluster through the portal — see [Enable monitoring for Kubernetes clusters](https://learn.microsoft.com/azure/azure-monitor/containers/kubernetes-monitoring-enable) and the [Monitor AKS](https://learn.microsoft.com/azure/aks/monitor-aks) overview. Once it's enabled, the per-request access log lines show up in the [`ContainerLogV2` table](https://learn.microsoft.com/azure/azure-monitor/containers/container-insights-logs-schema), so a query like `ContainerLogV2 | where PodName startswith "httpbin-gateway-approuting-istio"` gives you every request the gateway served across all replicas.
