...
- Under the "Container Service" Namespace, chose metric eg: API server memory percentage
  ![Control plane Metrics!](Control-plane-metrics.png)
...
- If your AKS cluster already has Managed Prometheus enabled, then installing Azure Container Storage on that cluster will automatically start collecting Azure Container Storage metrics.
  ![AcStor Metrics!](acstor-grafana.png)
...
- To query and store Kubernetes events beyond 1 hour, enable the **Container Insights** add-on for deeper visibility into your cluster health.
  ![AutoRepair!](autorepair-events.png)
...
![Autoinstrumentation!](app-insights-overview-screenshot.png)
...
- See detailed steps for enabling [Autoinstrumentation for AKS](https://learn.microsoft.com/azure/azure-monitor/app/kubernetes-codeless).
...
- Choose the **Managed Prometheus Visualizations (Preview)** option to begin exploring consolidated insights and metrics.
  ![monitoring-toc-1!](monitoring-toc-1.png)
  ![monitoring-toc-2!](monitoring-toc-2.png)
...
- You can customize the replica count settings directly through your AKS cluster settings. See [Horizontal Pod Autoscaling (HPA) for Collector Replica set](https://learn.microsoft.com/azure/azure-monitor/containers/prometheus-metrics-scrape-autoscaling).
