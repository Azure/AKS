...
1. To add an NVIDIA NC A100 node pool running on Azure Linux to your AKS
cluster using the fully managed GPU node experience you can follow these
instructions. Please note, the following parameters must be specified in
your `az aks nodepool add` command to create an NVIDIA NC A100 node pool
running on Azure Linux:

* `--os-sku AzureLinux`: provisions a node pool with the Azure Linux
container host as the node OS.
* `--node-vm-size Standard_nc24ads_A100_v4`: provisions a node pool using the
`Standard_nc24ads_A100_v4` VM size. 
   * Please note, any of the sizes in the Azure `NC_A100_v4` series are
   supported: `Standard_NC24ads_A100_v4`, `Standard_NC48ads_A100_v4`, or
   `Standard_NC96ads_A100_v4`.
2. With the DCGM exporter installed by default, you can monitor and visualize
detailed GPU metrics such as utilization, memory consumption, and error states.
For more information on enabling and consuming GPU metrics in AKS, including
integration with Azure-managed or self-hosted Prometheus and Grafana, visit
the [GPU Getting Started](https://aka.ms/aks/gpu-default-experience) and
[Managed GPU Metrics](https://aka.ms/aks/managed-gpu-metrics) on AKS
documentation.
