...
Deploying a GPU workload isn’t just about picking the right VM-SKU. There is also significant operational overhead that developers and platform engineers need to manage. 
...
If you prefer not to use a preview feature, you can follow [these instructions](https://aka.ms/aks/use-gpus) on AKS to create an NVIDIA NC A100 node pool with Azure Linux by manually installing the NVIDIA device plugin via a DaemonSet. You’ll also need to manually install the DCGM exporter to consume GPU metrics.
...
With the DCGM exporter now managed on AKS, cluster operators can collect detailed GPU metrics such as utilization, memory consumption, and error states, and expose them to Prometheus for analysis. These metrics integrate naturally with existing observability pipelines, providing a foundation for intelligent automation and alerting.
...
For more information on enabling and consuming GPU metrics in AKS, including integration with Azure-managed or self-hosted Prometheus and Grafana, visit these [docs](https://aka.ms/aks/managed-gpu-metrics).

### What's next?

The Azure Linux and AKS teams are actively working on expanding support for additional GPU VM sizes and managed GPU features on AKS. You can expect to see Azure Linux support for the NVIDIA [ND A100](https://learn.microsoft.com/azure/virtual-machines/sizes/gpu-accelerated/ndma100v4-series), [NC H100](https://learn.microsoft.com/azure/virtual-machines/sizes/gpu-accelerated/ncadsh100v5-series), and [ND H200](https://learn.microsoft.com/azure/virtual-machines/sizes/gpu-accelerated/nd-h200-v5-series) families landing in the near future, as well as Azure Linux support for managed AKS GPU features like [multi-instance GPU (MIG)](https://learn.microsoft.com/azure/aks/gpu-multi-instance) and [KAITO](https://learn.microsoft.com/en-us/azure/aks/ai-toolchain-operator).
