...
- **Reduced memory consumption**: Your API server uses significantly less memory when handling large list requests. This reduces the likelihood of OOM events, thereby improving API server latency.
...
When the API server remains unstable after scaling to the maximum control plane resource limits, and out-of-memory (OOM) incidents continue, AKS applies a [managed flow schema and priority level configuration](https://learn.microsoft.com/troubleshoot/azure/azure-kubernetes/create-upgrade-delete/troubleshoot-apiserver-etcd?tabs=resource-specific#cause-4-aks-managed-api-server-guard-was-applied) that throttles non-system-critical API server calls. 
