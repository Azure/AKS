...
- **Drastically reduced conntrack table entries**: With LocalDNS running as a node-level service, DNS queries from pods are resolved locally, significantly reducing the number of DNS requests that need to traverse the node's network stack and create conntrack entries. This dramatically reduces pressure on the node's conntrack table, lowering the risk of resource exhaustion and related networking issues.
