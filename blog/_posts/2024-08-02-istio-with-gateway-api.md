...
Kubernetes is great at orchestrating containers, but it can be a bit tricky to manage traffic routing. There are many options and implementations that you, as a cluster operator has probably had to deal with. We have the default Services resource that can be used to expose applications within the cluster, but it is limited to routing based on layer 4 (TCP/UDP) and does not support more advanced traffic routing use cases. So enter the Ingress controller, which enabled layer 7 (HTTP) routing, and securing the North-South traffic using TLS, but it was not standardized and each vendor implementation learning a new set of resource annotations. When it came to managing and securing East-West traffic between services, we had to rely on a service mesh which is yet another layer of infrastructure to manage on top of Kubernetes. And we were in the same boat with configuration management with each vendor having their own custom resources ways of doing things.
...
The API has been designed from the ground up, taking into account the lessons learned from the Ingress and Service Mesh APIs. To me, it is a game-changer because ensures that you can implement express traffic routing configurations that were only possible via custom Ingress annotations. It also offers role-oriented configuration, which means that cluster operators can configure gateways and app developers can manage routes through the gateway. This is a big win for organizations that have a separation of duties between cluster operators and app developers. And finally, the API is portable, meaning that the API specification is supported by many implementations including [Istio](https://istio.io/latest/).
...
AKS is a managed Kubernetes offering and as such many popular open-source projects are available as managed add-ons and extensions. [Istio is one of those add-ons that can be enabled on AKS](https://learn.microsoft.com/azure/aks/istio-about?WT.mc_id=containers-146848-pauyu). AKS also provides managed deployments of Istio Ingress Gateway (both internal and external) which can be used to route traffic into the cluster. In this article, I'll show you how you can experiement with the Gateway API to manage the Istio external ingress gateway on AKS.
...
1. Provision [AKS cluster with Istio enbled](https://learn.microsoft.com/azure/aks/istio-deploy-addon#install-mesh-during-cluster-creation?WT.mc_id=containers-146848-pauyu)
