...
### Announcements
* Starting with Kubernetes version 1.26, in-tree persistent volume types `kubernetes.io/azure-disk` and `kubernetes.io/azure-file` are deprecated and will no longer be supported ([see more](https://learn.microsoft.com/azure/aks/csi-storage-drivers#migrate-custom-in-tree-storage-classes-to-csi)). A new `ValidatingAdmissionPolicy` has been added to block in-tree drivers' creation on AKS clusters with version >= 1.35.
...
* The following control plane components have been updated:
  * etcd has been updated to v3.5.22‑5 across all supported AKS versions.
  * ccp-initializer and customer-net-probe have been updated to version master.260112.1 across all supported AKS versions.
