...
* AKS now enables the kubelet configuration file path by default for Kubernetes 1.36 and later clusters. Instead of primarily relying on command-line flags, kubelet settings will now be applied through the kubelet configuration file. This aligns with the [Kubernetes](https://kubernetes.io/docs/tasks/administer-cluster/kubelet-config-file/) direction of moving kubelet configuration away from command-line flags and toward the structured kubelet config file.
* Updating a cluster from service principal authentication to [managed identity](https://aka.ms/aks/managed-identity-update) now triggers node reimages across agent pools. Configure [Node Disruption Policy](https://aka.ms/aks/nodedisruptionpolicy) to control when reimages that can disrupt workloads are allowed.
