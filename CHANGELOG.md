...
> Auto-generated draft of upcoming changes since the [2026-03-05 release](https://github.com/Azure/AKS/releases/tag/2026-03-05). This section covers merged PRs through daily branch `official/v20260407`. Last updated: 2026-04-08.
> 
...
* Deprecating API versions `2023-10-02-preview`, `2023-11-02-preview`, `2024-01-02-preview`, and `2024-02-02-preview` from manifests. Customers using these API versions should migrate to newer versions. [PR#15084298](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15084298) (AI-flagged: deprecation)
* [Kubernetes 1.29](https://learn.microsoft.com/azure/aks/supported-kubernetes-versions) has been deprecated and is no longer available for new cluster creation or upgrades. Kubernetes 1.32 has moved to [Long Term Support (LTS)](https://learn.microsoft.com/azure/aks/long-term-support) only — it is no longer available as a standard support version. Customers on 1.29 should upgrade to a supported version. [PR#15259250](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15259250) (PM: @kaarthis)
...
* [Istio CNI](https://learn.microsoft.com/azure/aks/istio-cni) for the [Istio-based service mesh add-on](https://learn.microsoft.com/azure/aks/istio-about) is now generally available, no longer requiring the `IstioCNIPreview` feature flag registration. Istio CNI provides a more secure alternative to the default init container-based approach for configuring pod traffic redirection. [PR#15161003](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15161003) (AI-flagged: new-feature)
* [AzureContainerLinux](https://learn.microsoft.com/azure/aks/cluster-configuration#azure-linux-container-host-for-aks) now supports ARM64 VM sizes with Trusted Launch enabled. [PR#15188854](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15188854) (PM: @allyford)

* Updated [Istio-based service mesh add-on](https://learn.microsoft.com/azure/aks/istio-about) revisions asm-1-27 and asm-1-28 to v1.27.8 and v1.28.5 respectively, addressing [CVE-2026-26308](https://nvd.nist.gov/vuln/detail/CVE-2026-26308) and [CVE-2026-26311](https://nvd.nist.gov/vuln/detail/CVE-2026-26311). Revision asm-1-26 has been deprecated. Users can restart workload pods to trigger re-injection of the updated istio-proxy version. See [patch upgrade details](https://learn.microsoft.com/azure/aks/istio-upgrade#patch-version-upgrade). [PR#15033621](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15033621) (PM: @mitchconnors)
* Users can now customize `terminationGracePeriodSeconds` on [Istio Kube Gateway](https://learn.microsoft.com/azure/aks/istio-about) proxy pods, allowing fine-tuned control over graceful shutdown behavior for gateway workloads. [PR#15239634](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15239634) (PM: @mitchconnors)
* [Fleet managed namespaces](https://learn.microsoft.com/azure/kubernetes-fleet/howto-managed-namespaces) now support PATCH operations, allowing users to update individual properties (e.g., annotations or labels) via Azure CLI without needing to re-specify all existing properties. [PR#15242336](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15242336) (AI-flagged: api-improvement)

#### Preview features

* [NAT Gateway v2](https://learn.microsoft.com/azure/aks/nat-gateway) is now available as a preview outbound type in supported public cloud regions. NAT Gateway v2 provides improved scalability and reliability for cluster egress traffic. [PR#15293836](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15293836) (PM: @cozhang)
...
* Added API-level validation to reject mTLS transit encryption on Kubernetes versions below 1.34, as mTLS requires Cilium 1.18+ which is only available on K8s 1.34+. [PR#15016001](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15016001) (AI-flagged: validation-change)
...
* Enabled new [Long Term Support (LTS)](https://learn.microsoft.com/azure/aks/long-term-support) billing behavior across all regions. LTS clusters now use updated billing logic for more accurate usage tracking. [PR#15177807](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15177807) (PM: @kaarthis)

* Azure Monitor services (Container Insights, Managed Prometheus, Application Insights) are transitioning to a cluster extension-based backend. This change is non-disruptive and does not change user experience or require customer action. See [migration details](https://aka.ms/coreextensionmigration). [PR#15244475](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15244475) (PM: @ponatara)
* Relaxed HTTP proxy `noProxy` validation to only check for invalid characters, and limited strict validation to new cluster creation only, preventing existing clusters with non-standard `noProxy` entries from being blocked during updates. [PR#15178031](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15178031) (PM: @allyford)
* [HTTP proxy configuration](https://learn.microsoft.com/azure/aks/http-proxy) now limits the number of TrustedCA certificates to 20. Clusters exceeding this limit will receive a validation error during create or update operations. [PR#15244285](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15244285) (PM: @allyford)
* [Control plane scaling profile](https://learn.microsoft.com/azure/aks/control-plane-scaling) now properly validates attempts to disable the scaling profile. Sending an empty or null `scalingSize` in an update request now returns an error instead of being silently ignored. [PR#15236957](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15236957) (AI-flagged: validation-change)
* [Deployment safeguards](https://learn.microsoft.com/azure/aks/deployment-safeguards) VAP controllers now only manage ValidatingAdmissionPolicies that are specific to the configured PSS level (e.g., Baseline or Restricted), rather than acting on all `aks-managed-*` VAPs. This prevents the controller from interfering with customer-created VAPs that happen to use AKS naming conventions. [PR#15302014](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15302014) (AI-flagged: behavioral-change)
* [Fleet](https://learn.microsoft.com/azure/kubernetes-fleet/overview) update run strategies now treat empty string values for `MaxConcurrency` and `MaxAllowedFailures` as equivalent to null (unset) in API versions `2026-02-01-preview`, `2026-03-02-preview`, and `2026-05-01-preview`. Previously, empty strings and null values could produce different behavior during serialization. [PR#15301681](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15301681) (AI-flagged: behavioral-change)
#### Bug fixes

* Fixed an issue in the [Istio-based service mesh add-on](https://learn.microsoft.com/azure/aks/istio-about) where CRD installer pods could fail to schedule on nodes tainted with `CriticalAddonsOnly`, improving installation and upgrade reliability. [PR#15129806](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15129806) (PM: @mitchconnors)
* Fixed [a bug](https://github.com/Azure/AKS/issues/5640) in the AKS-managed `nodes/proxy` ValidatingAdmissionPolicy on AKS Automatic clusters where RBAC rules containing only `nonResourceURLs` were incorrectly denied. [PR#14970814](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/14970814) (PM: @wangamanda)
* Fixed an issue where HTTP proxy `noProxy` entries with a valid trailing dot (e.g., `example.com.`) were incorrectly rejected during cluster validation. [PR#15097768](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15097768) (AI-flagged: bug-fix)
* Fixed an issue where [VPA](https://learn.microsoft.com/azure/aks/vertical-pod-autoscaler) admission controller pods served stale TLS certificates after AKS certificate rotation, causing VPA to silently stop functioning. [PR#15059681](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15059681) (AI-flagged: bug-fix)
* Re-enabled the same Key Vault check when enabling KMS, preventing customers from accidentally using a shared Key Vault which could cause issues. [PR#15020170](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15020170) (AI-flagged: bug-fix)
* Fixed an issue where Fleet cluster mesh members could not be deleted while in the `Connecting` state. [PR#15090025](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15090025) (AI-flagged: bug-fix)
* Fixed a day-0 bug where Karpenter could pick up an expired bootstrap token from a stopped agent pool, causing new node provisioning to fail. [PR#15091039](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15091039) (AI-flagged: bug-fix)
* Added support for Application Load Balancer (ALB) identity exemption in NRG lockdown deny assignments, preventing ALB operations from being blocked by node resource group lockdown. [PR#15007857](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15007857) (AI-flagged: bug-fix)
* Improved error handling to return a clear `400 BadRequest` error for non-zonal node pools when the requested VM size is restricted in all availability zones, instead of a confusing CRP failure. [PR#15096609](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15096609) (AI-flagged: bug-fix)
* Fixed Application Gateway Ingress Controller (AGIC) add-on subnet validation errors for subnet-by-ID scenarios. [PR#15037462](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15037462) (AI-flagged: bug-fix)
* Fixed a subnet validation bug in the AGIC add-on. [PR#14989204](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/14989204) (AI-flagged: bug-fix)
* Improved performance of user-defined routing (UDR) subnet validation by parallelizing checks, preventing ARM timeouts on clusters with many subnets. [PR#15008920](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15008920) (AI-flagged: bug-fix)
* Fixed Istio add-on resource limits to allow smaller resource usage configurations as requested by customers. [PR#15000622](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15000622) (AI-flagged: bug-fix)
* Fixed a race condition during [Fleet](https://learn.microsoft.com/azure/kubernetes-fleet/overview) membership creation where concurrent attempts to join the same cluster to different fleets could silently succeed, potentially leaving the cluster in the wrong fleet. [PR#15113084](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15113084) (AI-flagged: bug-fix)
* Fixed VM size validation to allow custom sizes even when the CRP available sizes list is empty. [PR#15114185](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15114185) (AI-flagged: bug-fix)
* Fixed an upgrade race condition where getting a VM InstanceView could return a 404 error for a VM that is being deleted, causing upgrade failures. [PR#15149532](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15149532) (AI-flagged: bug-fix)
* Fixed a latent bug where kubelet feature gate removal during upgrades used the cluster-level Kubernetes version instead of each agent pool's version, which could cause kubelet startup failures when upgrading pools individually. [PR#15148050](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15148050) (AI-flagged: bug-fix)
* Fixed control plane provisioning to always backfill the control plane IP address, preventing cluster deadlock when a prior operation failed mid-provisioning. [PR#15138515](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15138515) (AI-flagged: bug-fix)
* Fixed an issue on [network isolated clusters](https://learn.microsoft.com/azure/aks/concepts-network-isolated) where Windows package sync could fail if the Windows download URL was not set. [PR#15152545](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15152545) (AI-flagged: bug-fix)
* Fixed an issue where Azure CNI clusters with PodSubnet configuration could not be created due to a logic error in the custom VNet validator. [PR#15219348](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15219348) (AI-flagged: bug-fix)

* Added a [Validating Admission Policy](https://learn.microsoft.com/azure/aks/use-admission-controllers) to block Kubernetes Service objects from binding the Azure WireServer IP (`168.63.129.16`), preventing a potential security risk where malicious users could hijack WireServer traffic via Service `clusterIP`, `externalIPs`, or `loadBalancerIP` fields. [PR#15242514](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15242514) (PM: @shasb)
* Fixed an issue where NVMe OS disk size validation incorrectly used 2048 GiB as the upper limit instead of 2040 GiB, causing VMSS creation failures for clusters using NVMe-enabled VM sizes with large OS disks. [PR#15219277](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15219277) (AI-flagged: bug-fix)
* Improved the error message for `InsufficientSubnetSize` failures to include a direct link to the troubleshooting guide, helping users diagnose and resolve subnet sizing issues more quickly. [PR#15177428](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15177428) (AI-flagged: bug-fix)

* Fixed an issue where blob container names with underscores could cause storage operations to fail. [PR#15270952](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15270952) (AI-flagged: bug-fix)
* Fixed a bug where rollback validation incorrectly blocked changes to the [node OS upgrade channel](https://learn.microsoft.com/azure/aks/auto-upgrade-node-os-image). Customers can now change their node OS upgrade channel without being blocked by rollback checks. [PR#15190064](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15190064) (AI-flagged: bug-fix)
* Improved error messages when AKS operations fail because all schedulable nodes are cordoned. Previously, the `ControlPlaneAddOnsNotReady` error was surfaced as a generic internal error; it now returns an actionable client error indicating that nodes are unschedulable. [PR#15166548](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15166548) (AI-flagged: bug-fix)
* Fixed a regression where `networkPluginValidator` was not called during cluster creation, potentially allowing invalid network plugin configurations. [PR#15224129](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15224129) (AI-flagged: bug-fix)
* Improved CRP (Compute Resource Provider) error enrichment to preserve the original inner error message, providing more detailed error information when VMSS operations fail. [PR#15276271](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15276271) (AI-flagged: bug-fix)
* Improved error messaging when credential retrieval fails on clusters with [disabled local accounts](https://learn.microsoft.com/azure/aks/manage-local-accounts-managed-azure-ad). The error now includes a troubleshooting guide link to help users resolve the issue. [PR#15272265](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15272265) (AI-flagged: bug-fix)
* Fixed an issue where deleting a [maintenance configuration](https://learn.microsoft.com/azure/aks/planned-maintenance) did not properly clean up the associated auto-upgrade schedule, potentially causing unexpected upgrade attempts. [PR#15194766](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15194766) (AI-flagged: bug-fix)
* Fixed a 401 authentication error when listing VMs for HOBO (Hosted on Behalf Of) clusters during agent pool get/list requests. The fix skips unnecessary VM list calls for HOBO clusters where the ARM endpoint is not applicable. [PR#15306396](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15306396) (AI-flagged: bug-fix)
#### Component updates
...
* Updated AgentBaker to [v0.20260304.0](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/14948459). [PR#14948459](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/14948459) (PM: @allyford)
* Updated Container Insights agent to version 3.1.34. [PR#14960197](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/14960197) (PM: @VIMEHT)
* Updated `ama-metrics` with a Metrics Extension upgrade to fix OTel histogram empty bucket handling. [PR#14961849](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/14961849) (PM: @aritraghosh)
* Updated Azure NPM image to v1.6.42, resolving CVEs identified in v1.6.39. [PR#15101674](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15101674) (AI-flagged: component-update)
* Updated Azure CNI and CNS to v1.7.15. [PR#15102749](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15102749) (AI-flagged: component-update)
* Updated [KAITO](https://learn.microsoft.com/azure/aks/ai-toolchain-operator) to v0.9.3. [PR#15037204](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15037204) (AI-flagged: component-update)
* Updated Application Gateway Ingress Controller (AGIC) add-on to version 1.9.16. [PR#14987718](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/14987718) (AI-flagged: component-update)
* Available Kubernetes patch versions updated: 1.35.1, 1.34.4, 1.33.8, 1.32.12, 1.31.14. See [Kubernetes release notes](https://kubernetes.io/releases/) for upstream fixes in each version. [PR#15145142](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15145142) (PM: @kaarthis)
* Kubernetes patch versions 1.32.12 and 1.31.14 have been removed from availability. Customers using these specific patch versions should upgrade to newer patch releases. [PR#15175572](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15175572) (PM: @kaarthis)
* Updated Container Insights agent toggles, cleaning up versions 3.1.34 and older. Increased memory limits for Container Insights cx component per ICM 755696559. [PR#15147048](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15147048) (PM: @VIMEHT)
* Updated addon-token-adapter to master.260322.2 to fix CVEs. [PR#15146595](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15146595) (AI-flagged: cve-fix)
* Updated admissions enforcer image to master.260323.1 to fix CVEs (ICM 760594983). [PR#15148079](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15148079) (AI-flagged: cve-fix)
* Updated SGX attestation image (confcom/ACCSGXDevicePlugin addon) to version 3.3.2, addressing CVEs and upgrading to Ubuntu 22.04 base image with PSW 2.28.100. [PR#15187156](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15187156) (AI-flagged: component-update)
* Updated kube-proxy images across all supported Kubernetes versions (1.31, 1.32, 1.33, 1.34, 1.35). [PR#15082268](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15082268) (AI-flagged: component-update)
* Updated [Flux](https://learn.microsoft.com/azure/azure-arc/kubernetes/tutorial-use-gitops-flux2) helm-controller and source-controller to address CVEs. [PR#15188016](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15188016) (AI-flagged: cve-fix)

* Updated [Microsoft Defender for Containers](https://learn.microsoft.com/azure/defender-for-cloud/defender-for-containers-introduction) `security-publisher` from 1.1.57 to 1.1.59 and `low-level-collector` from 2.1.109 to 2.1.110 (AKS >= 1.35) / 2.0.242 to 2.0.243 (AKS 1.29–1.34), remediating multiple CVEs. [PR#15176777](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15176777) (PM: @eyalgur)
* Updated AgentBaker to [v0.20260318.1](https://github.com/Azure/AgentBaker/compare/v0.20260318.0...v0.20260318.1) for weekly SIG release. [PR#15192772](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15192772) (PM: @allyford)
* [Istio-based service mesh add-on](https://learn.microsoft.com/azure/aks/istio-about) now supports revision asm-1-29 with Istio [1.29.1](https://istio.io/latest/news/releases/1.29.x/announcing-1.29/). To adopt the new revision, follow the [Istio add-on upgrade guide](https://learn.microsoft.com/azure/aks/istio-upgrade). Restart workloads to trigger sidecar re-injection of the updated istio-proxy version. [PR#15179391](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15179391) (PM: @phwilson)
* [Node Auto Provisioning (Karpenter)](https://learn.microsoft.com/azure/aks/node-autoprovision) has been updated to version `v1.10.2-aks`. [PR#15271657](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15271657) (PM: @wilsondarko)
* [Application routing add-on](https://learn.microsoft.com/azure/aks/app-routing) (aks-app-routing-operator) has been updated to version `0.2.20`. [PR#15240719](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15240719) (AI-flagged: component-update)
* [KEDA](https://learn.microsoft.com/azure/aks/keda-about) operator memory limits increased to 3000Mi, reducing risk of OOM-related restarts under high scaling load. [PR#15280873](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15280873) (AI-flagged: component-update)
* [Application Gateway Ingress Controller](https://learn.microsoft.com/azure/aks/app-gateway-ingress-controller) has been updated to version `1.9.8`. [PR#15172923](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15172923) (AI-flagged: component-update)
* Open Service Mesh has been updated to `v1.2.14`. [PR#15270542](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15270542) (AI-flagged: component-update)
* Updated AgentBaker to [v0.20260330.0](https://github.com/Azure/AgentBaker/releases/tag/v0.20260330.0) for weekly SIG release. [PR#15261618](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15261618) (PM: @allyford)
* Updated [Microsoft Defender for Containers](https://learn.microsoft.com/azure/defender-for-cloud/defender-for-containers-introduction) `security-publisher` from 1.1.57 to 1.1.59 and `low-level-collector` from 2.1.109 to 2.1.110 (AKS >= 1.35) / 2.0.242 to 2.0.243 (AKS 1.29–1.34), remediating CVEs including CVE-2025-15558, CVE-2026-24051, CVE-2026-33186, CVE-2026-25679, CVE-2026-27142, and CVE-2026-27139. [PR#15282284](https://dev.azure.com/msazure/CloudNativeCompute/_git/aks-rp/pullrequest/15282284) (AI-flagged: cve-fix)

<!-- REVIEWED_PR_IDS: 14947881, 14947943, 14948048, 14948058, 14948132, 14948219, 14948221, 14948315, 14948390, 14948412, 14948459, 14948517, 14948695, 14948903, 14948927, 14949073, 14949448, 14949583, 14949604, 14949827, 14950136, 14950305, 14950314, 14950397, 14950506, 14950614, 14950746, 14950760, 14950814, 14950849, 14951000, 14951254, 14951322, 14951384, 14951569, 14951747, 14951876, 14952098, 14952139, 14953028, 14953496, 14954053, 14954369, 14954862, 14955073, 14955272, 14955992, 14956027, 14956036, 14956820, 14956825, 14957195, 14957443, 14957554, 14957568, 14957898, 14958702, 14
* [Windows Server Annual Channel (Preview)](https://aka.ms/aks/windows-annual-channel-retirement) on AKS will be retired on May 15, 2026, please transition to the [Long Term Servicing Channel (LTSC)](https://aka.ms/aks/upgrade-windows-os-version) by that date. From now to May 15, 2026 you can continue to use Windows Server Annual Channel (Preview) without disruption. On May 15, 2026, AKS will no longer produce new Windows Server Annual Channel node images or provide security patches. You will not be able to create new node pools with Windows Server Annual Channel. On May 15, 2027, AKS will remove all existing Windows Server Annual Channel node images, which will cause scaling and remediation (reimage and redeploy) operations to fail.
...
### Behavioral Changes
* Nodes are now annotated with a `kubernetes.azure.com/security-patch-timestamp` annotation during a security VHD reboot upgrade. This gives you a unified way to verify when the last security patch was applied to each node. Refer to [Autoupgrade Node OS Image FAQs](https://learn.microsoft.com/azure/aks/auto-upgrade-node-os-image?tabs=azure-cli#node-os-autoupgrades-faq) for more information.
* By default, AKS no longer creates or updates Network Security Groups on subnets it delegates for [Application Gateway for Containers](https://learn.microsoft.com/azure/application-gateway/for-containers/quickstart-deploy-application-gateway-for-containers-alb-controller-addon), improving reliability in policy-managed environments.
* To protect against potential security concern of [remote code execution via nodes/proxy get permission](https://kubernetes.io/docs/reference/access-authn-authz/kubelet-authn-authz/#kubelet-authorization:~:text=Warning%3A,executing%20commands%20in%20any%20container%20running%20on%20the%20node.), AKS Automatic has added multiple layers of defense:
...
* [Gateway API CRDs](https://learn.microsoft.com/azure/aks/managed-gateway-api) can now be enabled directly without first requiring a supported gateway implementation such as the [Managed Istio service mesh add-on](https://learn.microsoft.com/azure/aks/istio-about) to be enabled on the cluster.

### Component Updates
* Konnectivity has been updated to [v0.31.4-6](https://github.com/kubernetes-sigs/apiserver-network-proxy/releases/tag/v0.31.4) to resolve CVEs: [CVE-2025-61729](https://nvd.nist.gov/vuln/detail/CVE-2025-61729), [CVE-2025-61727](https://nvd.nist.gov/vuln/detail/CVE-2025-61727)
...
* Kube-egress-gateway has been updated to [v0.1.4](https://github.com/Azure/kube-egress-gateway/releases/tag/v0.1.4) to address multiple CVEs:  [CVE-2025-4673](https://nvd.nist.gov/vuln/detail/CVE-2025-4673), [CVE-2025-58183](https://nvd.nist.gov/vuln/detail/CVE-2025-58183), [CVE-2025-47914](https://nvd.nist.gov/vuln/detail/CVE-2025-47914), [CVE-2025-58188](https://nvd.nist.gov/vuln/detail/CVE-2025-58188), [CVE-2025-47906](https://nvd.nist.gov/vuln/detail/CVE-2025-47906), [CVE-2025-58181](https://nvd.nist.gov/vuln/detail/CVE-2025-58181), [CVE-2025-58185](https://nvd.nist.gov/vuln/detail/CVE-2025-58185), [CVE-2025-22874](https://nvd.nist.gov/vuln/detail/CVE-2025-22874), [CVE-2025-61723](https://nvd.nist.gov/vuln/detail/CVE-2025-61723), [CVE-2025-47912](https://nvd.nist.gov/vuln/detail/CVE-2025-47912), [CVE-2025-58187](https://nvd.nist.gov/vuln/detail/CVE-2025-58187), [CVE-2025-61724](https://nvd.nist.gov/vuln/detail/CVE-2025-61724), [CVE-2025-0913](https://nvd.nist.gov/vuln/detail/CVE-2025-0913), [CVE-2025-61725](https://nvd.nist.gov/vuln/detail/CVE-2025-61725), [CVE-2025-58189](https://nvd.nist.gov/vuln/detail/CVE-2025-58189), [CVE-2025-47907](https://nvd.nist.gov/vuln/detail/CVE-2025-47907), [CVE-2025-8058](https://nvd.nist.gov/vuln/detail/CVE-2025-8058), [CVE-2025-64329](https://nvd.nist.gov/vuln/detail/CVE-2025-64329), and [CVE-2024-25621](https://nvd.nist.gov/vuln/detail/CVE-2024-25621)
* Application Monitoring has been updated to [v1.1.0](https://github.com/microsoft/appmonitoring-k8s/releases/tag/1.1.0). [Learn more](https://learn.microsoft.com/azure/azure-monitor/app/kubernetes-codeless?tabs=portal).
...
* Istio-based service mesh add-on revision `asm-1-25` has been deprecated. Revision `asm-1-28` is now supported. See the [Istio add-on upgrade documentation](https://learn.microsoft.com/azure/aks/istio-upgrade) for details on how to upgrade mesh revisions.
...
* Ubuntu version updates
  * Ubuntu 24.04 is now generally available and will be the default for OS SKU `Ubuntu` starting in Kubernetes v1.35. This means that if you upgrade to Kubernetes v1.35 with `Ubuntu` OS SKU, you'll automatically update your OS version from Ubuntu 22.04 to Ubuntu 24.04. If you'd like to continue to use Ubuntu 22.04, you can use it until Kubernetes v1.36 end of life. You can also create or update your existing node pools using CLI version 2.82.0+. For more information, see [documentation](https://aka.ms/aks/upgrade-os-version).
...
* AKS has now published the results from the [CIS Kubernetes Benchmark v1.12.0](https://www.cisecurity.org/benchmark/kubernetes/) recommendations on AKS. The results are applicable to AKS 1.32.x through AKS 1.34.x. Detailed report is available in [documentation](https://learn.microsoft.com/azure/aks/cis-kubernetes).
* AKS has now published the results from the [CIS Ubuntu 24.04 LTS Benchmark v1.0.0](https://www.cisecurity.org/benchmark/ubuntu_linux). Detailed report is available in [documentation](https://aka.ms/aks/ubuntu-cis).
...
* AKS LTS (Long Term Support) patch versions are now available:
  * Kubernetes 1.30.101-akslts - [Changelog](https://github.com/aks-lts/kubernetes/blob/release-1.30-lts/CHANGELOG/CHANGELOG-1.30.md)
  * Kubernetes 1.29.101-akslts - [Changelog](https://github.com/aks-lts/kubernetes/blob/release-1.29-lts/CHANGELOG/CHANGELOG-1.29.md)
  * Kubernetes 1.28.103-akslts - [Changelog](https://github.com/aks-lts/kubernetes/blob/release-1.28-lts/CHANGELOG/CHANGELOG-1.28.md)
...
* OpenTelemetry support for AKS monitoring is now in limited public preview. Sign up form can be found [here](https://aka.ms/AzureMonitorOTelPreview)
* [Private IP support for Static Egress Gateway](https://learn.microsoft.com/azure/aks/configure-static-egress-gateway#static-private-ip-support-preview) is now available in public preview on clusters of version >=1.34.
...
* [Identity bindings](https://learn.microsoft.com/azure/aks/identity-bindings-concepts) is now available in preview. This addresses [workload identity's](https://learn.microsoft.com/azure/aks/workload-identity-overview?tabs=dotnet) scale limitation of 20 federated identities credentials per managed identity.
...
* Revised experience for [data encryption at rest for secrets in AKS using KMS provider for Azure Key Vault](https://learn.microsoft.com/azure/aks/kms-data-encryption-concepts) with options for platform-managed keys or customer-managed keys is now available in preview.
...
* [Windows Exporter](https://learn.microsoft.com/en-us/azure/aks/windows-best-practices#monitoring)) is now installed by default in some regions.
