...
* Azure Service Mesh add-on revision `asm-1-30` (estimated to release in early July) introduces the following changes to default behavior:
    * The default proxy redirection mechanism will change from privileged init containers to Istio CNI for all revision `asm-1-30` and above. Read more about [Istio CNI](https://learn.microsoft.com/azure/aks/istio-cni). To retain the existing proxy redirection mechanism, see the [instructions to disable Istio CNI](https://learn.microsoft.com/azure/aks/istio-cni#disable-istio-cni).
