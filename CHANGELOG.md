...
* AKS is blocking creation of new clusters using Availability Sets (VMAS) effective September 30, 2025. Existing VMAS clusters must migrate to Virtual Machine Scale Sets (VMSS) before the retirement date. For more information, see [Availability Sets retirement](https://learn.microsoft.com/azure/aks/availability-sets-on-aks#availability-sets-retirement-date).
* AKS is now blocking creation of new clusters with Basic Load Balancer following the complete retirement announcement. Affected clusters must migrate to Standard Load Balancer. For more information, see [Basic Load Balancer retirement announcement](https://learn.microsoft.com/en-us/answers/questions/1033471/retirement-announcement-basic-load-balancer-will-b).
...
* Kubelet Serving Certificate Rotation is now enabled by default in Sovereign cloud regions. Existing node pools in these regions will have KSCR enabled by default when they perform their first upgrade to any Kubernetes version 1.27 or greater. New node pools on Kubernetes version 1.27 or greater will have KSCR enabled by default. For more information, see [Kubelet Serving Certificate Rotation](https://aka.ms/aks/kubelet-serving-certificate-rotation). Track regional rollout at [#4986](https://github.com/Azure/AKS/issues/4986).
...
* Advanced Container Networking now enforces Layer 7 (L7) policy restrictions in Cilium network policies. This is achieved through validating admission policies and bindings. When an L7 policy is applied without the feature enabled, the policy application will be denied and display a message to help identify the misconfiguration.
...
* Customers in ussec and usnat regions will start using ama-logs MSI mode when they create new clusters, providing enhanced functionality. Existing clusters are not impacted. This follows the deprecation announcement for legacy authentication mode.
...
* [Node Auto Provisioning](https://learn.microsoft.com/azure/aks/node-autoprovision) default AKSNodeClasses will now use Ubuntu 22.04 for Kubernetes versions < 1.34 and Ubuntu 24.04 for Kubernetes versions 1.34+. This ensures consistency across AKS node image defaults. This does not affect existing clusters' default AKSNodeClasses.
* The hostpath volume Validation Admission Policy (VAP) is now more flexible. An explicit allowlist of container images is now allowed to mount a hostpath volume, rather than blanket denying pods that specify a hostpath volume. The allowlist includes fluent-bit images from the `mcr.microsoft.com/oss` path and Geneva mdsd images used by the Azure Backup extension. Additional system-excluded namespaces include: `azappconfig-system`, `azureml`, `dapr-system`, `dataprotection-microsoft`, `flux-system`, `acstor`, `sc-system`, and `azure-extensions-usage-system`.
* Starting with Kubernetes version 1.33, clusters using Azure CNI Powered by Cilium will include a new AKS-managed component named iptables monitor. This component is a sidecar container alongside the Cilium agent that will set labels on the Cilium node if a user iptables rule is detected.
* The metric `ingress_upstream_latency_seconds` has been [removed](https://github.com/kubernetes/ingress-nginx/releases/tag/controller-v1.12.0) following its deprecation upstream.
* `InPlaceOrRecreate` feature gate enabled for VPA(Vertical Pod Autoscaler) 1.4.2 in AKS 1.34, allowing customers to use the `InPlaceOrRecreate` update mode in their VPA objects. The feature is alpha in VPA `1.4.2` and must be explicitly enabled; it does not change the default update mode (auto), so only customers who opt in by specifying `InPlaceOrRecreate` will be affected. For more details on the feature: [Official feature doc](https://kubernetes.io/blog/2025/05/16/kubernetes-v1-33-in-place-pod-resize-beta/), [Upstream release note](https://github.com/kubernetes/autoscaler/releases/tag/vertical-pod-autoscaler-1.4.0).
* Azure CNI and CNS have been updated to version [1.7.4](https://github.com/Azure/azure-container-networking/releases/tag/v1.7.4) with behavior change of Pod Subnet * Dynamic IP Allocation will SNAT Azure DNS traffic (168.63.129.16) using the node IP instead of the pod CIDR IP, aligning the behavior with Static Block Allocation.
...
* Azure Monitor Metrics (ama-metrics) has been updated to the [release-09-29-2025](https://github.com/Azure/prometheus-collector/blob/rashmi/fix-config-processing-test-pipeline/RELEASENOTES.md#release-09-29-2025).
...
* App monitoring add-on has been updated to 1.0.0-beta.9, which contains OTEL enablement for Microsoft distros for the upcoming public preview.
