...
* If you use any programming/scripting logic to list and select minor version of Kubernetes before creating clusters with the `ListKubernetesVersions` API, note that starting from Kubernetes v1.27, the API returns `SupportPlan` as `[KubernetesOfficial, AKSLongTermSupport]`. Please ensure you update any logic to exclude `AKSLongTermSupport` versions to avoid any breaks and choose `KubernetesOfficial` support plan versions.  Otherwise, if LTS is indeed your path forward please first opt-into the Premium tier and the `AKSLongTermSupport` support plan versions from the `ListKubernetesVersions` API before creating clusters.  Refer [long term supprt](https://learn.microsoft.com//azure/aks/long-term-support) for more information.
* AKS patch version 1.29.4 is now available.

### Release Notes

* Features:
  * With this release, Azure Linux 2.0 is a supported OS for AKS Long Term Support (LTS) with v1.27. Learn more about [Azure Linux and LTS](https://aka.ms/aks-azurelinux-lts).
...
* Bug Fixes:
  ...
  * Fixed an issue where the `agentPoolProfiles` property was getting wiped on CNI Overlay clusters using pod identity.
